...
/*
Originally comes from Meteor Rejects
...
public class AutoFarmPlus extends Module {
    ...
    private final Setting<Boolean> offhand = sgPlant.add(new BoolSetting.Builder()
        .name("offhand")
        .description("Puts the crops in your offhand.")
        .defaultValue(true)
        .visible(plant::isVisible)
        .build()
    );
    ...
    private boolean till(BlockPos pos, Block block) {
        ...
        if (moist && tillable && mc.world.getBlockState(pos.up()).isAir()) {
            ...
            return interact(pos, hoe, rotate.get());
        }
        return false;
    }
    ...
    private boolean plant(BlockPos pos, Block block) {
        ...
        if (onlyReplant.get()) {
            ...
            if(plantItems.get().contains(mc.player.getOffHandStack().getItem()) && mc.player.getOffHandStack().getItem() != Items.NETHER_WART) {
                findItemResult = new FindItemResult(36, 1);
            }
    ...
    private boolean bonemeal(BlockPos pos, BlockState state, Block block) {
        ...
        return interact(pos, bonemeal, rotate.get());
    }
    ...
    public boolean interact(BlockPos pos, FindItemResult findItemResult, boolean rotate) {
